...
export default function EarlyLearningPage() {
  ...
  const { data: allBlogs, isLoading } = useBlogs("Early Learning");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-muted/20">
      {/* Hero Section */}
      <Banner data={earlyLearningBannerData} />
      ...
      <div className="container mx-auto px-4 py-5">
        ...
        <Tabs
          ...
          onValueChange={(val) => setActiveSubcategory(val)}
          className="w-full"
        >
          {allBlogs.length > 0 && (
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-5">
              <TabsList className="inline-flex h-auto p-1.5 bg-primary/10 backdrop-blur overflow-x-auto">
                <TabsTrigger
                  value="all"
                  className="px-6 py-2 whitespace-nowrap data-[state=active]:bg-background data-[state=active]:shadow-sm"
                >
                  All Posts
                </TabsTrigger>
                {subcategories.map((subcategory) => (
                  <TabsTrigger
                    key={subcategory}
                    value={subcategory}
                    className="px-6 py-2 whitespace-nowrap capitalize data-[state=active]:bg-background data-[state=active]:shadow-sm"
                  >
                    {subcategory}
                  </TabsTrigger>
                ))}
          ...
          {/* Blog Grid */}
          <TabsContent value={activeSubcategory} className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredBlogs?.map((blog: TBlog) => (
                <Link key={blog._id} href={`/early-learning/${blog.slug}`}>
                  <BlogCard blog={blog} />
                </Link>
              ))}
            </div>
